...
public:
    ...
    inline void drawControl(DonutStyle::ControlElement element, const QStyleOption* opt, QPainter* p, const QWidget* w) const;
...
class DonutStyleHelp
{
public:
    inline DonutStyleHelp(const QStyle* style = nullptr);

    inline void setStyle(const QStyle* style);

    inline const QStyle* qStyle() const;
    inline const DonutStyle* myStyle() const;
