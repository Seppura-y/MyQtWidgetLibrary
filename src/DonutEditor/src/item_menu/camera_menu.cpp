...
{
    setStyleSheet(ConfigHelper::GetQssString(":/MediaManager/res/css/camera_menu.css"));
    ...
    lw_camera_->setStyleSheet(ConfigHelper::GetQssString(":/MediaManager/res/css/item_list_widget.css"));
    ...
    lw_local_file_->setStyleSheet(ConfigHelper::GetQssString(":/MediaManager/res/css/item_list_widget.css"));
    ...
    lw_media_src_->setStyleSheet(ConfigHelper::GetQssString(":/MediaManager/res/css/item_list_widget.css"));
...
{
    MediaSrcDialog* dia = new MediaSrcDialog();
    Qt::WindowFlags flag = dia->windowFlags();
    dia->setWindowFlags(flag | Qt::MSWindowsFixedSizeDialogHint);

    QObject::connect(dia, &MediaSrcDialog::sigMediaDialogProcessScreenCap, this, [this] { emit sigProcessScreenCapture(); });
    QObject::connect(dia, &MediaSrcDialog::sigMediaDialogProcessScreenCapFinished, this, [this] { qDebug() << "menu fin"; emit sigProcessScreenCaptureFinished(); });
    ...
    {
        //ItemListWidget* lwCur = (ItemListWidget*)ui.tw_item->currentWidget()->layout()->itemAt(0)->widget();
        //lwCur->item_type_ = (int)ItemListType::ITEM_LIST_TYPE_MEDIA_SRC;
        //key = lwCur->currentItem()->text();
        //obj = *config_tools_->GetObject(ConfigHelper::JsonObjType::JSONOBJ_TYPE_MEDIA_SRC);
    ...
    for (;;)
    {
        if (dia->exec() == QDialog::Accepted)
        {
            auto media_info = dia->getMediaInfo();
            qDebug() << media_info.resolution.c_str() << " : " << media_info.pix_format.c_str();
            qDebug() << media_info.width << " : " << media_info.height;
            qDebug() << media_info.type;


            info.insert("name", QString(media_info.src.c_str()));
            info.insert("resolution", QString(media_info.resolution.c_str()));
            info.insert("x", QString(media_info.x));
            info.insert("y", QString(media_info.y));
            info.insert("width", QString(media_info.width));
            info.insert("height", QString(media_info.height));
            info.insert("cap_type", QString(media_info.type));
            ...
            info.insert("capture_type", cap_type);
            ...
            config_tools_->WriteJson(QString(media_info.src.c_str()), info, ConfigHelper::JsonObjType::JSONOBJ_TYPE_MEDIA_SRC);
            config_tools_->SaveJson(ITEM_LIST_CONFIG);
            InitItemList();
            break;
        }
        else if (dia->close())
        {
            ...
            config_tools_->WriteJson(key, obj, ConfigHelper::JsonObjType::JSONOBJ_TYPE_MEDIA_SRC);
            ////QJsonDocument doc;
            ////doc.setObject(obj);
            ////QByteArray arr = doc.toJson();
            config_tools_->SaveJson(ITEM_LIST_CONFIG);
