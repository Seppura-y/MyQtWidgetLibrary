...
void Title::initUi()
{
	...
	setStyleSheet(ConfigHelper::GetQssString(":/title/res/css/title.css"));
}
...
{
	int id = QFontDatabase::addApplicationFont(":/title/res/fonts/Font Awesome 6 Pro-Light-300.otf");
	if (id < 0)
...
{
	//MediaManager* window = qobject_cast<MediaManager*>(watched);
	//if (window == nullptr)
	//	return false;
...
void Title::onWindowMaximized()
{
	ui->pb_max->setText(QChar(0xf24d));
}

void Title::onWindowNormal()
{
	ui->pb_max->setText(QChar(0x23f9));
}
